...
Program::Program() {
    ...
    for (int i = 0; i < 30; i++) {
        float x = 250 + 50 * (i%10);    //fixed rows
        float y = 200 + 50 * (i/10);
    ...
    if(score>=lifeupscore && lives<5){
        ...
        for (Projectile& p : Projectile::projectiles) { 
            ...
            if(p.ID!=0 && HitBox::Collision(p.getHitBox(),player->hitBox)){
                PlayerReset();  //Fixed proyectiles hitting
            }
        }
...
void Program::Draw() {
    ...
    DrawText(("Score: " + std::to_string(score)).c_str(), 10, 10, 20, WHITE); //Para que se vea el score
...
void Program::KeyInputs() {
    ...
    if (IsKeyPressed('K')) score += 500; //so when you press ’K’ the score increases by 500
...
void Program::Reset() {
    ...
    for (int i = 0; i < 30; i++) {
        ...
        Enemy::enemies.push_back(std::pair<std::pair<float, float>, Enemy*> {
            std::pair<float, float>{x, y}, 
            new StdEnemy(x, y)
        }); //enemies are now re-added to the next iteration of the game
    }
}